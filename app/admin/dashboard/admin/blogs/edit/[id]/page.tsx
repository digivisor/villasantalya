...
const [initialTitle, setInitialTitle] = useState<string>('');
  ...
  useEffect(() => {
    const loadBlogPost = async () => {
      ...
      try {
        ...
        setTimeout(() => router.push('/admin/dashboard/admin/blogs'), 1500);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return `https://api.villasantalya.com${path}`;
  }
  ...
  const validate = () => {
    ...
    if (!(imagePreview || formData.image)) errors.image = 'Blog görseli gereklidir';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Görsel değiştiyse (yeni görsel yüklendiyse) FormData ile gönder
      if (formData.image) {
        body = new FormData();
        ...
        // Multer ile FormData kullanırsan Content-Type otomatik olur!
      } else {
        // Sadece metin güncelleniyorsa
      ...
      if (!res.ok) {
        ...
        throw new Error(r.error || 'Blog yazısı güncellenemedi');
      }
